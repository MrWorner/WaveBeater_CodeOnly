...
public class UI_DeadScreen : MonoBehaviour
{
    ...
    private static UI_DeadScreen _instance;
    #endregion Поля

    #region Свойства
    public static UI_DeadScreen Instance => _instance;
    ...
    {
        ColoredDebug.CLog(gameObject, "<color=red>UI_DeadScreen:</color> Запускаю анимацию экрана смерти.", _coloredDebug);
        _background.DOFade(_backgroundAlpha, _backgroundFadeDuration).OnComplete(() =>
        {
            ...
            ColoredDebug.CLog(gameObject, "<color=red>UI_DeadScreen:</color> Панель сдвигается на позицию <color=yellow>{0}</color>.", _coloredDebug, _shownPosition);
        });
        ColoredDebug.CLog(gameObject, "<color=red>UI_DeadScreen:</color> Фон затемняется.", _coloredDebug);
    }
    ...
    {
        ColoredDebug.CLog(gameObject, "<color=red>UI_DeadScreen:</color> Нажата кнопка перезапуска. Запускаю анимацию выхода.", _coloredDebug);
        ...
        {
            ColoredDebug.CLog(gameObject, "<color=red>UI_DeadScreen:</color> Панель скрылась. Перезагружаю сцену.", _coloredDebug);
